...
# Configurations are loaded from the .env file with defaults
...
try:
    ...
    if not API_ID or not API_HASH or not SUDO_USER:
...
def is_on_cooldown(user_id: int) -> bool:
    """Checks if a user is on command cooldown."""
...
def load_persistent_data():
    """Loads all persistent data from files on startup."""
...
if BOT_TOKEN:
    ...
    MENU_TEXTS = {
        ...
        "auto_mod": STATIC_MENU_ADMIN.split('---')[2].strip(),
        "afk": STATIC_MENU_ADMIN.split('---')[3].strip(),
        "user_admin": STATIC_MENU_ADMIN.split('---')[4].strip(),
    }
    USER_BUTTONS = [
        [Button.inline("🎵 Media", b"menu_media"), Button.inline("🛠️ Tools & Fun", b"menu_tools")],
        [Button.inline("⚙️ Utility", b"menu_utility"), Button.inline("Close Menu", b"menu_close")]
    ]
    ADMIN_BUTTONS = [
        [Button.inline("🛡️ Moderation", b"menu_moderation"), Button.inline("🔧 Auto-Mod", b"menu_auto_mod")],
...
async def block_unblock_handler(event):
    ...
    try:
        if command == "block":
            await client(BlockRequest(reply_msg.sender_id)); await event.edit("`User blocked.`")
        elif command == "unblock":
            await client(UnblockRequest(reply_msg.sender_id)); await event.edit("`User unblocked.`")
    except Exception as e: await event.edit(f"🚫 **Error:** {e}")
...
@client.on(events.NewMessage(pattern=r'^/listusers(?:\s|$)'))
async def list_users(event):
    ...
    for uid in sorted(list(AUTH_USERS)):
        ...
        except Exception: name = "Unknown User"
        msg += f"🔹 [{name}](tg://user?id={uid})"; msg += " `(Sudo)`" if uid == SUDO_USER else ""; msg += "\n"
