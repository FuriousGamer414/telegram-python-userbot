...
## ----------------------------------------------------------------------------------------------------------------
## --- UTILITY COMMANDS (FOR ALL AUTH USERS) ---
## ----------------------------------------------------------------------------------------------------------------
...
async def worker_handler(event):
    ...
    if command == "new":
        ...
        save_workers_state()
        await event.edit(f"✅ Worker `{name}` created successfully.")
    elif command == "list":
...
@client.on(events.NewMessage(pattern=r'^/(ytmp3|ytmp4|play|fbmp4)(?:\s|$)'))
    ...
    try:
        ...
        if command_name == '/play':
            ...
            try:
                ...
                await handle_download_request(event, video_url, file_type, status_msg, "youtube")
            except Exception as e: await status_msg.edit(f"🚫 Search Error: {e}"); print(f"Error in /play command search: {e}")
        elif command_name == '/fbmp4':
            if not re.match(FACEBOOK_REGEX, query): return await status_msg.edit("🚫 Invalid Facebook URL format.")
            await handle_download_request(event, query, "mp4", status_msg, "facebook")
        else:
            file_type = "mp3" if command_name == '/ytmp3' else "mp4"
            if not re.match(YOUTUBE_ID_REGEX, query): return await status_msg.edit("🚫 Invalid YouTube URL format.")
            await status_msg.edit("⏳ **Processing URL...**")
            await handle_download_request(event, query, file_type, status_msg, "youtube")
...
async def handle_download_request(event, url: str, file_type: str, status_msg, source: str):
    try:
        ...
        if source == "youtube": api_endpoint = "youtube/videofhd" if file_type == "mp4" else "youtube/audio"
        elif source == "facebook": api_endpoint = "facebook/video"
        else: return await status_msg.edit("🚫 Unknown download source.")
        ...
        await client.send_file(event.chat_id, cached_file_path, caption=f"**Title:** `{title}`\n**Quality:** `{quality}`", thumb=thumb_path, attributes=attrs)
